...
- **Slack MCP 설정 필수** (commit-업데이트 채널에 PR 알림을 보내기 위해 필수)
...
**Slack MCP** (필수): 이 템플릿은 commit-업데이트 채널에 자동 PR 알림을 보내기 위해 Slack MCP가 필요합니다. 다음이 필요합니다:
...
- `/commit-push-pr` - **내 작업을 팀에 공유하기**
  ...
  **중요**: PR이 생성되거나 푸시될 때 자동으로 commit-업데이트 채널에 Slack 알림을 보냅니다. 메시지는 코딩을 모르는 팀원들도 쉽게 이해할 수 있도록 자연스러운 한국어로 작성됩니다.
