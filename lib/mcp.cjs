...
function getEnvVarDescription(varName) {
  const descriptions = {
    ...
    SUPABASE_PROJECT_REF: 'Supabase Project Reference (from dashboard)',
...
async function configureMcpServers(rl, platformInfo, githubConfig = null, supabaseConfig = null, vercelToken = null) {
  ...
  // Track which credentials we need for special handling
  ...
  for (const serverName of enabledServers) {
    ...
    // Special handling for Supabase project ref
    if (serverName === 'supabase' && credentials.SUPABASE_PROJECT_REF) {
      supabaseProjectRef = credentials.SUPABASE_PROJECT_REF;
    }
  ...
  for (const [serverName, serverConfig] of Object.entries(template.mcpServers)) {
    ...
    if (serverName === 'supabase' && config.type === 'http') {
      const projectRef = results.collectedEnvVars.SUPABASE_PROJECT_REF || supabaseProjectRef;
      if (projectRef) {
        config.url = `https://mcp.supabase.com/mcp?project_ref=${projectRef}`;
      }
    }
