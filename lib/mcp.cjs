...
async function configureMcpServers(rl, platformInfo, githubConfig = null, supabaseConfig = null, vercelToken = null) {
  ...
  // Supabase MCP is now always enabled (HTTP-based, no env vars needed)
  const supabaseMcpEnabled = true;
  const supabaseProjectRef = supabaseConfig?.projectRef || null;
  ...
  const defaultEnabled = ['filesystem', 'slack', 'supabase', 'context7', 'magic-ui', 'memory']; // Always enable core tools

  // Auto-enable GitHub if configured in GitHub setup step
  if (githubMcpEnabled) {
    defaultEnabled.push('github');
    log('GitHub MCP server enabled (configured in GitHub setup)', 'info');
  }

  log('Core MCP servers enabled: filesystem, slack (REQUIRED), supabase', 'info');
  log('Development tools enabled: context7, magic-ui, memory', 'info');
    ...
    for (const [categoryKey, category] of Object.entries(SERVER_CATEGORIES)) {
      ...
      for (const serverName of categoryServers) {
        ...
        if (serverName === 'supabase') {
          console.log(color(`  ${serverName}: REQUIRED (always enabled, authenticate via 'claude /mcp')`, 'dim'));
          continue;
        }
  ...
  for (const [serverName, serverConfig] of Object.entries(template.mcpServers)) {
    ...
    // Special handling for Supabase MCP - use cloud URL if project_ref available
    if (serverName === 'supabase' && supabaseProjectRef && config.type === 'http') {
  ...
  console.log(color('Use .claude/templates/mcp.template.json as the source template.', 'dim'));
  console.log('');
