...
/**
 * Create seed files with initial content (only if they don't exist)
 */
function createSeedFiles() {
  ...
  const seedFiles = CONFIG.seedFiles || [];
  ...
  for (const file of seedFiles) {
    ...
    try {
      if (fs.existsSync(filePath)) {
        ...
        log(`Created seed file: ${file.path}`, 'success');
      }
    } catch (error) {
      results.errors.push({ file: file.path, error: error.message });
      log(`Failed to create ${file.path}: ${error.message}`, 'error');
    }
  }

  if (results.skipped.length > 0) {
    log(`Seed files already exist: ${results.skipped.join(', ')}`, 'info');
  }
